  ...
  return (
    <div>
      ...
      {subTab === 'active' ? (
        ...
        /* Student Join Requests Sub-Tab */
        pendingStudents.length === 0 ? (
          <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--light-text-secondary)', backgroundColor: 'var(--light-card)', border: '1px solid var(--light-border)', borderRadius: '12px' }}>
            <FiUserPlus size={44} style={{ color: 'var(--light-text-muted)', marginBottom: '12px' }} />
            <h4 style={{ color: 'var(--light-text-primary)' }}>No pending student join requests</h4>
            <p style={{ fontSize: '13px', marginTop: '6px' }}>Share the student invite link with students to let them request to join.</p>
            {user?.role !== 'student' && (
              <button
                type="button"
